  ...
  return (
    <div className="mx-auto max-w-4xl mt-10 space-y-4">
      ...
      <div className="flex items-center gap-3 text-sm">
        <span className="text-slate-600 truncate">{owuiUrl || 'No URL configured (Settings → Apps)'}</span>
        {owuiUrl && (
          <a href={owuiUrl} target="_blank" rel="noopener noreferrer" className="rounded border px-2 py-1 hover:bg-slate-50">Open tool</a>
        )}
      </div>
      {offline && (
        <div className="rounded border border-amber-300 bg-amber-50 p-3 text-amber-900">Tool offline: Check Settings → Apps and verify the OWUI link.</div>
      )}
      {project && (
